...
namespace MetricsAgentTests.Controllers
{
    public class NetworkAgentControllerUnitTests
    {
        ...
        {
            _mock.Setup(repository => repository.Create(It.IsAny<NetworkMetricDto>())).Verifiable();
            ...
            _mock.Verify(repository => repository.Create(It.IsAny<NetworkMetricDto>()), Times.AtMostOnce());
        }

        [Fact]
        public void GetByPeriodFromAgent_ReturnsOk()
        {
            _mock.Setup(repo => repo.GetByTimePeriod(It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>())).Returns(new List<NetworkMetricDto>());
        ...
        {
            _mock.Setup(repo => repo.GetAll()).Returns(new List<NetworkMetricDto>());
