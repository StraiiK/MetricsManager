...
namespace MetricsAgentTests.Controllers
{
    public class RomAgentControllerUnitTests
    {
        ...
        {
            _mock.Setup(repository => repository.Create(It.IsAny<RomMetricDto>())).Verifiable();
            ...
            _mock.Verify(repository => repository.Create(It.IsAny<RomMetricDto>()), Times.AtMostOnce());
        }

        [Fact]
        public void GetByPeriodFromAgent_ReturnsOk()
        {
            _mock.Setup(repo => repo.GetByTimePeriod(It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>())).Returns(new List<RomMetricDto>());
        ...
        {
            _mock.Setup(repo => repo.GetAll()).Returns(new List<RomMetricDto>());
