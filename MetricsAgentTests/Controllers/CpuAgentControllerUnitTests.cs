...
namespace MetricsAgentTests.Controllers
{
    public class CpuAgentControllerUnitTests
    {
        ...
        //{
        //    _mock.Setup(repository => repository.CreateAsync(It.IsAny<CpuMetricDto>())).Verifiable();

        //    _controller.CreateAsync(new CpuMetricCreateRequest
        //    {
        //        Time = DateTimeOffset.FromFileTime(1),
        //        Value = 50
        //    });

        //    _mock.Verify(repository => repository.CreateAsync(It.IsAny<CpuMetricDto>()), Times.AtMostOnce());
        //}

        //[Fact]
        //public void GetByPeriodFromAgent_ReturnsOk()
        //{
        //    _mock.Setup(repo => repo.GetByTimePeriodAsync(It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>())).Returns(new List<CpuMetricDto>());

        //    var result = _controller.GetByPeriodAsync(DateTimeOffset.FromFileTime(1), DateTimeOffset.FromFileTime(100));
        ...
        //{
        //    _mock.Setup(repo => repo.GetAllAsync()).Returns(new List<CpuMetricDto>());
