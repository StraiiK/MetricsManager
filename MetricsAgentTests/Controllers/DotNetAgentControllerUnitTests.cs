...
namespace MetricsAgentTests.Controllers
{
    public class DotNetAgentControllerUnitTests
    {
        ...
        {
            _mock.Setup(repository => repository.Create(It.IsAny<DotNetMetricDto>())).Verifiable();
            ...
            _mock.Verify(repository => repository.Create(It.IsAny<DotNetMetricDto>()), Times.AtMostOnce());
        }

        [Fact]
        public void GetByPeriodFromAgent_ReturnsOk()
        {
            _mock.Setup(repo => repo.GetByTimePeriod(It.IsAny<DateTimeOffset>(), It.IsAny<DateTimeOffset>())).Returns(new List<DotNetMetricDto>());
        ...
        {
            _mock.Setup(repo => repo.GetAll()).Returns(new List<DotNetMetricDto>());
